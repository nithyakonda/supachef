/*
  ...
  1. Changes
    - Add new `meal_recipes` column as jsonb[]
    - Migrate existing `recipe_ids` data to new `meal_recipes` format
    - Drop old `recipe_ids` column
    - Update `process_suggested_recipes` function to work with new structure
    - Clean up old indexes

  2. Data Migration
    - Convert existing recipe_ids to meal_recipes format with recipe details
    - Set default values for flags (leftover, lunchbox, aiSuggested, isPlaceholder)
*/
...
-- Step 2: Migrate data from the old recipe_ids (uuid[]) to the new meal_recipes (jsonb[])
...
-- Step 3: Drop the old recipe_ids column
...
-- Step 5: Drop old indexes related to the removed columns (only if they exist)
...
-- Step 6: Add new index for meal_recipes column
