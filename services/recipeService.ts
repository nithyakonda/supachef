...
export const recipeService = {
  ...
  // Simplified Recipe Service that works without API endpoints
// Replace your existing recipeService import with this

import { Recipe } from '@/types';
import { clientSideRecipeParser } from './clientSideRecipeParser';

// Mock data for development (you can keep your existing sampleRecipes)
const MOCK_RECIPES: Recipe[] = [
  {
    id: '1',
    title: 'Butternut Soup with Avocado & Chickpeas',
...
export const simplifiedRecipeService = {
  ...
  toggleFavorite: async (id: string): Promise<Recipe | null> => {
    ...
    const updatedRecipe = { ...recipe, isFavorite: !recipe.isFavorite };
    await simplifiedRecipeService.updateRecipe(updatedRecipe);
  ...
  importFromUrl: async (
    ...
    const savedRecipe = await simplifiedRecipeService.createRecipe(recipeData);
  ...
  batchImportFromUrls: async (
    ...
    for (let i = 0; i < urls.length; i++) {
      ...
      try {
        const recipe = await simplifiedRecipeService.importFromUrl(url);
